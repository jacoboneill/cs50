...
person *create_family(int generations){
  // PERF: Allocate memory for new person
  ...
  if (generations > 1){
    ...
    // PERF: Set parent pointers for current person
    child -> parents[0] = parent0;
    child -> parents[1] = parent1;

    // PERF: Randomly assign current person's alleles based on the alleles of their parents
  ...
  // If there are no generations left to create
    ...
    // PERF: Set parent pointers to NULL
    child -> parents[0] = NULL;
    child -> parents[1] = NULL;

    // PERF: Randomly assign alleles
    child -> alleles[0] = random_allele();
    child -> alleles[1] = random_allele();

  }

  // PERF: Return newly created person
  return child;
}

// Free `p` and all ancestors of `p`.
void free_family(person *p){
  // PERF: Handle base case
  if(p -> parents[0] == NULL && p -> parents[1] == NULL){
    free(p);
    return;
  }

  // PERF: Free parents recursively
